...
<section class="vt-rooms-section">
    <div class="container">
        ...
        <p class="vt-section-sub">Klik salah satu ruangan di bawah untuk memulai tur virtual 3D</p>
        ...
        @else
            <div class="vt-rooms-grid">
                ...
                <div class="vt-room-card" onclick="openRoom3D({{ $room->id }})">
                    ...
                    <div class="vt-room-info">
                        ...
                        <div class="vt-room-meta">
                            <span>{{ count($room->media) ?? 0 }} media di dinding</span>
                            @if($room->door_link_type !== 'none')
                                <span>• Pintu: {{ $room->door_label ?: 'Aktif' }}</span>
                            @endif
                        </div>
                    </div>
                </div>
...
<div id="room3d-viewer" style="display:none; position:fixed; inset:0; z-index:9999; background:#111;">
    <div id="vt3d-topbar">
        <div id="vt3d-room-title">Ruangan Virtual</div>
        <div style="display:flex;align-items:center;gap:12px;">
            <div id="vt3d-view-btns" style="display:flex;gap:6px;">
                @foreach(['front'=>'Depan','left'=>'Kiri','right'=>'Kanan','back'=>'Belakang'] as $v=>$l)
                <button class="vt3d-view-btn" data-view="{{ $v }}">{{ $l }}</button>
                @endforeach
            </div>
            <button id="vt3d-close-btn" onclick="closeRoom3D()" title="Keluar">
    ...
    <div id="vt3d-hint">Gunakan tombol View untuk membalik sudut pandang, atau seret/drag untuk menggeser.</div>
</div>
...
<script>
    window.virtualRooms3D = {!! json_encode($virtual3dRooms->map(function($r) {
        return [
            'id'             => $r->id,
            'name'           => $r->name,
