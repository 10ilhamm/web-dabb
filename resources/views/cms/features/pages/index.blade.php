...
@section('breadcrumb_active', $feature->name . ' — Halaman')
...
<div class="space-y-6" x-data="pageManager()">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-800">{{ __('cms.feature_pages.title', ['name' => $feature->name]) }}</h1>
            ...
            <table class="w-full text-sm text-left">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide">{{ __('cms.feature_pages.col_title') }}</th>
                        <th class="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide text-center">{{ __('cms.feature_pages.col_sections') }}</th>
                        <th class="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide text-center">{{ __('cms.feature_pages.col_order') }}</th>
                        <th class="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide text-center">{{ __('cms.feature_pages.col_action') }}</th>
                    </tr>
                </thead>
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <button @click="openEditModal({{ $page->id }}, '{{ addslashes($page->title) }}', `{{ addslashes($page->description ?? '') }}`, {{ $page->order }})"
    ...
    <div x-show="addModal.open" x-cloak
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md z-[9999]"
            ...
            <form action="{{ route('cms.features.pages.store', $feature) }}" method="POST" class="px-6 py-5 space-y-4">
                @csrf
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('cms.feature_pages.form.title') }} <span class="text-red-500">*</span></label>
                    <input type="text" name="title" required placeholder="{{ __('cms.feature_pages.form.title_placeholder') }}"
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('cms.feature_pages.form.description') }}</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('cms.feature_pages.form.order') }} <span class="text-red-500">*</span></label>
                    <input type="number" name="order" min="0" value="{{ $feature->pages->count() + 1 }}" required
                        class="w-full px-3.5 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">
                </div>
    ...
    <div x-show="editModal.open" x-cloak
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md z-[9999]"
            ...
            <form :action="`{{ route('cms.features.pages.index', $feature) }}/${editModal.id}`" method="POST" class="px-6 py-5 space-y-4">
                @csrf
                @method('PUT')
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('cms.feature_pages.form.title') }} <span class="text-red-500">*</span></label>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm z-[9999] p-6"
            ...
            <div class="flex flex-col items-center text-center gap-4">
                ...
                <div class="flex items-center gap-3 w-full">
                    ...
                    <form :action="`{{ route('cms.features.pages.index', $feature) }}/${deleteModal.id}`" method="POST" class="flex-1">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
